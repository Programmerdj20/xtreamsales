...
export async function updateUserStatus(
    ...
    try {
        ...
        try {
            console.log(
                "Paso 1: Actualizando en la tabla profiles con supabaseAdmin"
            );
            ...
            if (profileError) {
                console.error(
                    "Error actualizando estado en profiles con supabaseAdmin:",
                    profileError
                );
                ...
                try {
                    ...
                    if (normalError) {
                        console.error(
                            "Error actualizando con cliente normal:",
                            normalError
                        );
                    } else {
                        console.log(
                            "Perfil actualizado correctamente con cliente normal"
                        );
                    }
                } catch (normalError) {
                    console.error(
                        "Excepción al actualizar con cliente normal:",
                        normalError
                    );
                }
            } else {
                console.log(
                    "Perfil actualizado correctamente con supabaseAdmin"
                );
            }
        } catch (profileUpdateError) {
            console.error(
                "Excepción al actualizar perfil:",
                profileUpdateError
            );
        }
        ...
        try {
            ...
            if (rpcError) {
                console.error(
                    "Error al usar RPC para actualizar estado:",
                    rpcError
                );
            } else {
        ...
        try {
            ...
            if (userData && userData.role === "reseller") {
                console.log(
                    "Usuario es revendedor, actualizando en tabla resellers"
                );

                // Actualizar con user_id usando supabaseAdmin
                try {
                    console.log(
                        "Actualizando con user_id usando supabaseAdmin"
                    );
                    ...
                    if (resellerError1) {
                        console.error(
                            "Error actualizando con user_id:",
                            resellerError1
                        );
                    } else {
                        console.log(
                            "Revendedor actualizado correctamente (user_id)"
                        );
                    }
                } catch (error1) {
                    console.error(
                        "Excepción actualizando con user_id:",
                        error1
                    );
                }
                ...
                try {
                    ...
                    if (resellerError2) {
                        console.error(
                            "Error actualizando con id:",
                            resellerError2
                        );
                    } else {
                        console.log(
                            "Revendedor actualizado correctamente (id)"
                        );
                    }
                } catch (error2) {
                ...
                console.log(
                    "Usuario no es revendedor, no es necesario actualizar la tabla resellers"
                );
            }
        } catch (userDataError) {
        ...
        try {
            console.log(
                "Paso 4: Verificando si es necesario crear el revendedor"
            );
            ...
            if (profileData && profileData.role === "reseller") {
                ...
                if (!existingReseller) {
                    ...
                    console.log(
                        "Revendedor ya existe, no es necesario crearlo"
                    );
                }
            }
        } catch (createError) {
