...
export const statsService = {
    async getDashboardStats(): Promise<DashboardStats> {
        ...
        try {
            ...
            console.log(
                "Obteniendo perfiles de revendedores desde profiles..."
            );
            ...
            if (profilesError) {
                console.error(
                    "Error al obtener perfiles con RPC:",
                    profilesError
                );
                resellersData = [];
            } else {
                ...
                const resellerProfiles =
                    profilesData?.filter((p) => p.role === "reseller") || [];
                console.log(
                    `Encontrados ${resellerProfiles.length} perfiles de revendedores`
                );
                console.log(
                    "Revendedores activos en profiles:",
                    resellerProfiles.filter((p) => p.status === "active").length
                );

                // Obtener datos adicionales de la tabla resellers
                const { data: resellersTableData, error: resellersError } =
                ...
                resellersData = resellerProfiles.map((profile) => {
                    ...
                    const resellerData = resellersTableData?.find(
                        (r) => r.id === profile.id || r.user_id === profile.id
                    );
                ...
                pendingResellersData = resellersData.filter(
                    (r) => r.status === "pending"
                );

                console.log("Datos combinados de revendedores:", {
                    total: resellersData.length,
                    activos: resellersData.filter((r) => r.status === "active")
                        .length,
            ...
            try {
                ...
                const { data: recentProfiles, error: recentProfilesError } =
                    await supabase
                ...
                console.log("Perfiles recientes obtenidos:", {
                    ...
                    error: recentProfilesError
                        ? recentProfilesError.message
                        : null,
                });

                if (recentProfilesError) {
                    console.error(
                        "Error al obtener perfiles recientes:",
                        recentProfilesError
                    );
                    recentResellers = [];
                } else {
                    // Filtrar solo los perfiles con rol 'reseller'
                    const resellerProfiles =
                        recentProfiles?.filter((p) => p.role === "reseller") ||
                        [];
                    console.log(
                        `Encontrados ${resellerProfiles.length} perfiles de revendedores recientes`
                    );

                    // Obtener datos detallados de la tabla resellers para estos perfiles
                    const resellerIds = resellerProfiles.map((p) => p.id);
                    const { data: resellerDetails, error: detailsError } =
                        await supabase
                            .from("resellers")
                            .select("*")
                            .in("id", resellerIds);
                    ...
                    recentResellers = resellerProfiles.map((profile) => {
                        // Buscar datos correspondientes en la tabla resellers
                        const resellerData = resellerDetails?.find(
                            (r) =>
                    ...
                    console.log(
                        `Devolviendo ${recentResellers.length} revendedores recientes combinados`
                    );
                }
            } catch (recentError) {
                console.error(
                    "Error al obtener revendedores recientes:",
                    recentError
                );
                recentResellers = [];
            }
        ...
        try {
            ...
            // Obtener todos los clientes (sin join por ahora para evitar errores)
            const { data: fetchedClientsData, error: clientsError } =
                await supabase
                    .from("clients")
                    .select("*")
                    .order("created_at", { ascending: false });
            ...
            if (clientsError) {
                ...
                // Procesar clientes para calcular estadísticas
                const processedClients = (clientsData || []).map((client) => {
                    // Calcular días restantes
                    const today = new Date();
                    const endDate = new Date(client.fecha_fin);
                ...
                clientsStats = {
                    ...
                    active: processedClients.filter(
                        (c) => c.status === "active"
                    ).length,
                    pending: 0, // Los clientes no tienen estado "pending"
                    recentRegistrations: processedClients
                        .slice(0, 5)
                        .map((client) => ({
                            id: client.id,
                            full_name: client.cliente,
                            reseller_name: "Admin", // Simplificado por ahora
                ...
                console.log("Estadísticas de clientes calculadas:", {
                    ...
                    active: clientsStats.active,
                    pending: clientsStats.pending,
                });
            }
        } catch (error) {
            console.error("Error al procesar estadísticas de clientes:", error);
        }
        ...
        try {
            ...
            const { data: pendingUsers, error: pendingUsersError } =
                await supabase.rpc("get_pending_users");

            console.log(
                "Usuarios pendientes encontrados con RPC:",
                pendingUsers,
                "Error:",
                pendingUsersError
            );
            ...
            if (pendingUsersError) {
                ...
                const { data: pendingProfiles, error: pendingProfilesError } =
                    await supabase
                        .from("profiles")
                ...
                console.log(
                    "Perfiles pendientes encontrados directamente:",
                    pendingProfiles,
                    "Error:",
                    pendingProfilesError
                );
                ...
                if (
                    ...
                    for (const profile of pendingProfiles) {
                        ...
                        console.log(
                            "Agregando alerta para perfil pendiente:",
                            profile
                        );
                        alerts.push({
                ...
                for (const user of pendingUsers) {
                    ...
                    console.log(
                        "Agregando alerta para usuario pendiente de RPC:",
                        user
                    );
                    alerts.push({
                        id: `pending-${user.id}`,
            ...
            for (const reseller of pendingResellersData) {
                ...
                console.log(
                    "Agregando alerta para revendedor pendiente:",
                    reseller
                );
                alerts.push({
        ...
        for (const reseller of expiringResellers) {
            try {
                ...
                alerts.push({
                    ...
                    message: `La suscripción de ${
                        reseller.full_name
                    } vence en ${endDate.toLocaleDateString()}.`,
                    severity: "medium",
                    timestamp: new Date().toISOString(),
                });
            } catch (error) {
        ...
        console.log(
            "Generando actividad reciente para revendedores:",
            recentResellers
        );
        ...
        console.log(
            `Generadas ${limitedActivity.length} actividades recientes`
        );
        ...
        console.log(
            `Revendedores por vencer (5 días): ${expiringSoonResellers.length}`
        );
        console.log(`Revendedores vencidos: ${expiredResellers.length}`);

        // Calcular clientes por vencer (7 días) y vencidos
        ...
        console.log(`Clientes por vencer (7 días): ${expiringSoonClients}`);
        console.log(`Clientes vencidos: ${expiredClients}`);
        ...
        const result = {
            resellers: {
                ...
                active:
                    resellersData.filter((r) => r.status === "active").length ||
                    0,
                pending:
                    resellersData.filter((r) => r.status === "pending")
                        .length || 0,
