...
import { PlusCircle, Users, UserCheck, AlertTriangle, UserX } from "lucide-react";
...
const ClientsPage = () => {
  ...
  const filterClients = (clientsList: ClientData[], search: string, status: string) => {
    const filtered = clientsList.filter((client) => {
      // Filtro por búsqueda
      const matchesSearch = search
        ? client.cliente?.toLowerCase().includes(search.toLowerCase()) ||
          client.whatsapp?.toLowerCase().includes(search.toLowerCase()) ||
          (client.usuario ? client.usuario.toLowerCase().includes(search.toLowerCase()) : false)
        : true;
  ...
  const handleRenewPlan = async (plan: string) => {
    ...
    try {
      ...
      toast.success(`Plan renovado exitosamente hasta ${endDate.toLocaleDateString()}`);
  ...
  const handleSendTemplate = async (template: Template) => {
    ...
    try {      
      ...
      const message = replaceVariables(template.content, {
        ...
        fecha_fin: clientToSend.fecha_fin ? new Date(clientToSend.fecha_fin).toLocaleDateString() : '',
      ...
      if (phone.startsWith('+')) {
        ...
        if (!phone.startsWith('52') && !phone.startsWith('57') && !phone.startsWith('54') && 
            !phone.startsWith('55') && !phone.startsWith('56') && !phone.startsWith('58') && 
            !phone.startsWith('51') && !phone.startsWith('50') && phone.length > 0) {
          phone = '52' + phone;
        }
      }
